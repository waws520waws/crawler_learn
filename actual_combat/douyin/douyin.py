...
import requests
from requests.adapters import HTTPAdapter  # 使用传输适配器
...
# 为啥就这么几个参数就能请求到数据（看网页中提交的还有加密参数）？
# url换成用户的，就请求失败，为啥？
# 看写的平台是android，我在PC端上会看到加密参数，是否是平台的原因？
QUERIES = {
...
def request_session():
    ...
    try:
        ...
        # 利用requests的HTTPAdapter重试机制，检测web服务器状态（通过判断响应码是否为200来判断服务器状态）
