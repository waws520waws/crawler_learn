...
- 会话（Session）跟踪是Web程序中常用的技术，用来跟踪用户的整个会话。常用的会话跟踪技术是Cookie与Session。
  Cookie通过在客户端记录（保存）信息来确定用户身份，Session通过在服务器端记录（保存）信息（用户会话所需的属性及其配置信息）来确定用户身份（session_id）。
...
- 什么是cookie
    ...
    - 分类
        - 会话Cookie：指存在浏览器内存的Cookie，当浏览器关闭，会话Cookie会失效
        - 持久Cookie：是保存在硬盘上的Cookie
- Cookie的工作原理
